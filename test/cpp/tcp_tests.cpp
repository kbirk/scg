...
// test_stream_flow_control_backpressure pushes many messages through a tiny
// per-stream window using the BLOCKING send. The window admits only a few
// messages at a time, so every send relies on the server replenishing credit
// (WINDOW_UPDATE) as its handler consumes. A correct credit loop delivers all
// messages losslessly; a broken one would deadlock or miscount.
void test_stream_flow_control_backpressure() {
	printf("Running Flow Control Backpressure test...\n");

	const int port = 18785;
	...
	cfg.transport = std::make_shared<scg::tcp::ServerTransportTCP>(tcfg);
	cfg.errorHandler = [](const scg::error::Error&) {};
	cfg.initialStreamWindow = 200; // bytes — only a handful of small messages fit
	auto server = std::make_shared<scg::rpc::Server>(cfg);
	pingpong::registerChatServer(server.get(), std::make_shared<ChatServerImpl>());
	TEST_CHECK(!server->start());
	std::this_thread::sleep_for(std::chrono::milliseconds(150));
	...
	pingpong::ChatClient chat(client);
	scg::context::Context ctx;
	auto ur = chat.upload(ctx);
	TEST_CHECK(!ur.second);
	auto upload = ur.first;
	...
	for (int32_t i = 1; i <= n; i++) {
		pingpong::ChatMessage m;
		m.seq = i;
		TEST_CHECK(!upload->send(m)); // blocks on credit; must not error
		want += i;
	}
	auto sr = upload->closeAndRecv();
	TEST_CHECK(!sr.second);
	TEST_CHECK(sr.first.total == want); // messages lost or miscounted => fail

	client->disconnect();
	server->shutdown();
	printf("Flow Control Backpressure test passed\n");
}

// test_stream_trysend exercises the non-blocking trySend: with credit available
// it sends immediately; the happy path integrates with a consuming server.
void test_stream_trysend() {
	printf("Running TrySend test...\n");

	const int port = 18786;
	...
	for (int i = 0; i < 100 && !sent; i++) {
		auto res = stream->trySend(m);
		TEST_CHECK(!res.second);
		sent = res.first;
		if (!sent) {
	...
	TEST_CHECK(echo.message.text == "echo:hi");

	stream->closeSend();
	client->disconnect();
	server->shutdown();
	printf("TrySend test passed\n");
}
...
TEST_LIST = {
	...
	TEST(test_stream_flow_control_backpressure),
	TEST(test_stream_trysend),
	{ NULL, NULL }
};

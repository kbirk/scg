...
class TCPTLSConnection : public rpc::Connection, public std::enable_shared_from_this<TCPTLSConnection> {
public:
    TCPTLSConnection(std::shared_ptr<ssl_socket> socket, bool noDelay = true)
        : socket_(std::move(socket)), closed_(false) {
            if (noDelay) {
                asio::ip::tcp::no_delay option(true);
                std::error_code ec;
                socket_->lowest_layer().set_option(option, ec);
            }
        }

    ~TCPTLSConnection() = default;
